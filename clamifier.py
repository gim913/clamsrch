...
class DataParser:
		...
		while line[idx] != '"':
			if line[idx] == '\\':
				...
				if line[idx] == '0':
					...
					self.values.append( int(ord(line[idx])) )
				else:
	...
	def parseNumbers(self, line):
		for number in line.split(','):
			...
			if cleanNumber[0:2] == "0x":
				val = int(cleanNumber, 16)
			else:
				val = int(cleanNumber, 10)
	...
	def strNumbers(self):
		idx = bisect.bisect([0x100, 0x10000, 0x100000000, 0x10000000000000000], self.big)
...
def bswap(val, bits):
	if bits == 16:
		...
		temp1 = bswap(val&0xffff, 16)
		temp2 = bswap((val>>16)&0xffff, 16)
		return ((temp1<<16) | temp2)
	else:
		temp1 = bswap( val&0xffffffff, 32 )
		temp2 = bswap( (val>>32)&0xffffffff, 32 )
...
class SigParser:
	...
	def parse(self):
		...
		# currently if "CRC:" is specified only one width is allowed
			...
			for Do_Reflect in [True, False]:
				bitWidth = list(self.typeDesc.values.keys())[0]
	...
	def dumpData(self, bitWidth, mode, dataObj):
		...
		for idx, elem in enumerate(dataObj.values):
			...
			tempBuf = form % (int(bitWidth / 4), elem)
			buff += tempBuf
			bufLen += len(tempBuf)
	...
	def dumpLogicalSignature(self, bitWidth, mode, dataObj):
		...
		# in the same order...
		clauses = []
		for idx,item in enumerate(logData):
			count = item
			if count > 1:
		...
		elements = []
		form = "%0*x"
		for elem,count in enumerate(logData):
			if (mode == Little_Endian) and (bitWidth != 8):
				elem = bswap(elem, bitWidth)
				
			elements.append( form % (int(bitWidth / 4), elem) )
